...
def send_confirmation_email(full_name: str, email_address: str, team: str) -> None:
    """Send confirmation through Gmail SMTP using a Gmail App Password."""
    ...
    smtp_username = _required_env("GMAIL_SMTP_USERNAME")
    smtp_password = _required_env("GMAIL_SMTP_APP_PASSWORD").replace(" ", "")
    from_address = os.environ.get("GMAIL_FROM_EMAIL", smtp_username).strip()
...
def index():
    ...
    try:
        with ASSIGNMENT_LOCK:
            ...
            if any(str(row.get("Email", "")).strip().lower() == email_address for row in existing_records):
                return render_error(
